(() => {
    ...
    // weirdly enough, it seems like if there are less than 4 style elements with a data-late-css
    // attribute, the page is either loading, or broken. this may change.
    let styleCount = document.documentElement?.innerHTML.match(/<style\s*(type="[^"]+"|)\s*data-late-css/g)?.length;
    ...
    let isLoading = (searchBoxClassCount || (refreshButton && !refreshLoadingIcon)) || (hasResults && feedScrolledToBottom && !isEnd) || styleCount < 4;
